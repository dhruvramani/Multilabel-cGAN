...
def generator_model():
    ...
    global x
    global y
    layer1 = Dense(350, input_dim = input_dim, activation='relu')(x)
    ...
    model = Model(input=x, output=layer5)
    return model
...
def generator_containing_discriminator(generator, discriminator):
    ...
    model = Model(input=x, output=[fake, disc_val])
    return model
...
def train():
    ...
    for epoch in range(num_epoch):
        ...
        for index in range(num_batches):
            ...
            print(output, end="")
            discriminator.trainable = False
            g_loss = disc_on_gen.train_on_batch(x_batch, [y_batch ,np.asarray([1] * batch_size, dtype=np.float32)])
            discriminator.trainable = True
